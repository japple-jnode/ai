...
function _mapUserFilePart(part, info) {
    const mediaType = part.mediaType ?? part.media_type;
    if (!mediaType) return null;

    if (!info.features.multimodalCapabilities.includes(mediaType)) {
        if (mediaType === 'text/plain' && part.data !== undefined) {
            return {
                type: 'text',
                text: Buffer.isBuffer(part.data) ? part.data.toString() : String(part.data)
            };
        }
        return null;
    }
    ...
    if (mediaType.startsWith('image/')) {
        return {
            type: 'image_url',
            image_url: {
                url: _fileDataUrl(part, mediaType)
            }
        };
    }
    ...
    if (mediaType === 'audio/wav' || mediaType === 'audio/mp3') {
        return {
            type: 'input_audio',
            input_audio: {
                data: Buffer.isBuffer(part.data) ? part.data.toString('base64') : part.data,
    ...
    if (mediaType === 'application/pdf' || mediaType === 'text/plain') {
        return {
            type: 'file',
            file: {
                file_data: _fileDataUrl(part, mediaType),
                filename: _fileName(part, mediaType)
            }
        };
    }
...
function _fileDataUrl(part, mediaType) {
...
function _fileName(part, mediaType) {
    if (typeof part.name === 'string' && part.name.length > 0) return part.name;
    if (typeof part.filename === 'string' && part.filename.length > 0) return part.filename;
    if (typeof part.uri === 'string' && part.uri.length > 0) {
        const clean = part.uri.split('?')[0];
        const base = path.basename(clean);
        if (base && base !== '.') return base;
    }
    if (mediaType === 'application/pdf') return 'file.pdf';
    if (mediaType === 'text/plain') return 'file.txt';
    return 'file';
}
...
function _schemaName(schema) {
    const title = schema?.title;
    if (typeof title === 'string' && title.length > 0) {
        return _toolApiName(title).slice(0, 64) || 'response';
    }
    return 'response';
}
...
function _extractToolCalls(message) {
    ...
    if (message.function_call?.name) {
        toolCalls.push({
            id: _fallbackToolCallId(_toolApiName(message.function_call.name)),
            type: 'function',
            function: {
...
function _messageText(message) {
    ...
    if (Array.isArray(message.content)) {
        for (let part of message.content) {
            ...
            else if (part?.type === 'refusal' && part.refusal) parts.push(part.refusal);
        }
    }
    if (typeof message.refusal === 'string' && message.refusal.length > 0) parts.push(message.refusal);

    return parts.join('');
}

function _toolCallX(id) {
    return {
        openaiChat: {
            toolCallId: id
        }
...
function _componentToolCallId(part) {
    return part?.x?.openaiChat?.toolCallId ?? part?.x?.openai_chat?.tool_call_id ?? part?.x?.tool_call_id;
}

function _fallbackToolCallId(name) {
    return `call_${name}`;
}

function _updateMeta(meta, data, choice) {
